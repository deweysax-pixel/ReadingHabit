...
const pickThemes = (value: string | string[] | undefined) => {
  if (Array.isArray(value)) {
...
export function SessionPageClient({ searchParams }: SessionPageClientProps) {
  ...
  const currentBook = {
    title: pickString(searchParams.bookTitle) ?? 'Lecture surprise',
    author: pickString(searchParams.bookAuthor) ?? 'Auteur mystère',
    theme: pickString(searchParams.bookTheme) ?? 'Inspiration',
  };

  const selectedThemes = useMemo(() => {
    const fallbackThemes = ['Leadership'];
    const themes = pickThemes(searchParams.themes);
    return themes.length ? themes : fallbackThemes;
  }, [searchParams.themes]);

  const rawMinutes = Number(pickString(searchParams.minutes));
            ...
            {GUIDED_QUESTIONS.map((question) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                  activeQuestion === question ? 'bg-midnight text-white shadow-lg' : 'bg-white/70 text-midnight hover:bg-white'
                }`}
              >
                {question}
              </button>
